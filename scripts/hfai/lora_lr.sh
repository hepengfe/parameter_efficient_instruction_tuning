...
for ((i=0; i<${#search_seq[@]}; i++))
    do
        ...
        tuning_setting="--tuning_mode ${peft_method} --lora_r ${peft_hp} --lora_alpha ${alpha} --learning_rate ${lr}"
        ...
        launch_command="${launch_prefix} prompt_tuning.py --model_name_or_path google/t5-xl-lm-adapt --model_arch encoder-decoder --per_device_train_batch_size 1 --per_device_eval_batch_size $eval_bs --eval_steps ${eval_save_steps} --save_steps ${eval_save_steps}  ${tuning_setting} --num_train_epochs 4 --dataset_name ni --data_dir ../../data/splits/${data_folder} --task_dir ../../data/tasks --predict_with_generate  --gradient_accumulation_steps 2 --do_train --logging_steps 500 --run_name $expr_name --logging_dir $expr_dir $launch_suffix"
